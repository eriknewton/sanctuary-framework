...
//
// Manual Arm / Disarm gate matrices. Fail-closed both ways: arm requires
// helper-ready AND daemon-up-with-policy; disarm is unconditional; auto-arm
// respects the operator-disarm latch.
//
...
final class ProtectionGateTests: XCTestCase {
    ...
    /// Disarm must ALWAYS be available to the operator - no System Settings
    /// trip should be required.
    ...
    // MARK: - disarmControlVisible (failed disarm keeps the exit path)

...
//
// Codex P2: the disarm state model. A disarm attempt is complete ONLY when a
// re-read proves the filter is off; every failure path (including the
// previously-swallowed loadFromPreferences error) must keep `disarmPending`
// set so the UI keeps a retryable Disarm control on screen. The model is a
// pure function (`disarmOutcome`) so these matrices run without
// NEFilterManager.
//
...
final class FilterConfigurationManagerTests: XCTestCase {
    ...
    /// The P2 exploit shape: saveToPreferences fails, the REAL filter may
    /// still be enabled. The state must reflect failure AND keep the disarm

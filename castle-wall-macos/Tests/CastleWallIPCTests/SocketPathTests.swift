...
final class SocketPathTests: XCTestCase {
    ...
    // MARK: - activeDaemonPresent (discovery probe)
    ...
    // MARK: - activeDaemonPresentForArming (arm-grade, spoof-proof)
    ...
    /// Forged active-config naming a live pid (the exact shape any local user
    /// can plant) must NOT satisfy arming - the real custody probe sees the
    /// test-owned (non-root) file and rejects it, even with a genuinely
    /// connectable socket behind it.

...
final class FilterConfigurationManager: NSObject, ObservableObject {
    ...
    /// Codex P2: true from the moment a disarm attempt starts until a re-read
    /// of the preferences PROVES `NEFilterManager.isEnabled == false`. While
    ...
    // MARK: - Disarm state model (pure, codex P2)
    ...
    /// Codex P2: every exit of this flow routes through `resolveDisarm`, and
    /// only a post-save re-read proving `isEnabled == false` clears

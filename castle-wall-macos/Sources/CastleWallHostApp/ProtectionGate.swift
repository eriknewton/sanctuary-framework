...
//
// Pure decision logic for the host app's manual Arm / Disarm control and the
// auto-arm re-evaluation path.
//
...
//     System Settings - the manual-control failure mode).
//
...
enum ProtectionGate {
    ...
    /// come from the authenticated arming-readiness probe: active-config
    /// discovery plus a successful pinned-key IPC handshake, never from
    /// discovery-grade `activeDaemonPresent`, whose legacy /tmp fallback any
    /// local user can forge.
    ...
    /// Disarm safety invariant: the control stays on screen while armed OR while the

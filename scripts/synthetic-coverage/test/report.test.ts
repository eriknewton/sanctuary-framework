...
describe("coverage report", () => {
  it("marks every matrix row as no_fixture with an empty registry", async () => {
    ...
    expect(report.summary.total_rows).toBe(20);
    expect(report.summary.rows_with_fixtures).toBe(0);
    expect(report.summary.rows_no_fixture).toBe(20);
    ...
    expect(markdown).toContain("Total rows: 20");
    expect(markdown).toContain("Rows without fixtures: 20");
  });
});

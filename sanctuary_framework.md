...
An engineering-only frame permits "secure enough" trade-offs: metadata leakage that's probabilistically unlikely to be exploited, context storage that's encrypted but platform-readable under subpoena, reputation systems that are pseudonymous but linkable with sufficient effort. The current landscape offers two prominent versions of "secure enough," and understanding why each falls short clarifies what Sanctuary requires.

**Ephemeral privacy is not sovereignty.** The most sophisticated platform privacy architectures — exemplified by Apple's Private Cloud Compute and Google's Private AI Compute — solve for data protection *during inference*. They are stateless and ephemeral: user data is processed in memory, never written to persistent storage, and encryption keys are destroyed after each request. This is genuine and impressive engineering. But it addresses a different problem than sovereignty. A participant's persistent cognitive state — the accumulated memory, preferences, strategies, and relational history that constitute the agent's knowledge of its principal — still resides within the platform's ecosystem, encrypted with the platform's key infrastructure, portable only within the platform's walled garden. Ephemeral inference privacy protects the act of answering a question. Cognitive sovereignty protects the ongoing relationship between a person and the agent that represents them. These platforms also offer no selective disclosure (no mechanism to prove a claim without revealing identity), no portable reputation (trust is locked to the ecosystem), and no architecture for autonomous agent operation (they model agents as services that respond to requests, not as persistent entities that reason over time).

**Local custody is not architectural sovereignty.** The local-first agent harness movement — exemplified by open-source frameworks that run agent orchestration, memory, and configuration on the user's own machine — solves for *physical custody* of agent state. This is a meaningful step: data that never leaves your device cannot be harvested by a platform. But physical custody without cryptographic protection is the difference between keeping valuables in your house and keeping them in a vault. Today's local agent frameworks store persistent state as plaintext files, with no encryption at rest, no integrity verification, no tamper detection, and no cryptographic delegation chains. Security research has documented average defense rates against attacks below 20%, plaintext credential storage, and supply-chain compromises injecting persistent backdoors into agent memory. Moreover, even local-first agents typically send their full context — including the principal's personal data, strategies, and preferences — to a remote model provider for inference, creating the very exposure that locality was supposed to prevent. Local is necessary but not sufficient for sovereignty.

For humans, the trade-offs embedded in both approaches are already unacceptable. A person whose agent's persistent memory is encrypted with platform-held keys has not achieved cognitive sovereignty — the platform can be compelled to access it, or can change its policies at will. A person whose local agent stores their financial strategies in a plaintext file accessible to any process on their machine has not achieved operational security. A person whose medical agent's reasoning can be subpoenaed has not achieved healthcare privacy. A person whose financial agent's metadata can be correlated has not achieved financial privacy. A person whose negotiation agent's identity can be linked across transactions has not achieved commercial privacy. "Secure enough" is a euphemism for "insecure in exactly the ways that matter."

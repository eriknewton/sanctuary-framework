...
/// Slice-A, unknown-identity leg: first boot admits nobody, leaving an owned,
/// jump-free table with an UNKNOWN confined history. The second boot admits
/// [`TEST_AGENT_UID`]; with no history for this boot, `plan_admitted_binding`
/// returns `RefuseWithNet` before any write-ahead runs, which routes through
/// the slice-A refusal's `install_net=true` arm with unknown history resolving
/// HostWide (the forced write-ahead failure is armed as a belt for the
/// known-history sibling below and is not what reaches this arm here). With the seam armed, the host-wide install must
/// be SKIPPED entirely: the pre-existing table is left exactly as the first
/// boot made it, never mutated into a deny-all net.
#[test]
fn a162_slicea_boot_stop_after_resolution_skips_hostwide_net() {
    ...
    // but drop the `confined` key to UNKNOWN.
    ...
    // Second boot: a CONFINING manifest, the write-ahead forced to fail (which
    // reaches the slice-A refusal's install-net arm), and the seam armed so a
    // stop is observed once this boot's scope resolution has already run.
    write_confining_manifest(dir.path(), &signing);
...
/// Slice-A, known-identity sibling: the first boot runs without a confining
/// manifest and the test then plants [`TEST_AGENT_UID`] in this boot's history
/// (see the body), so the journal names it before the second boot. The second boot's forced
/// write-ahead failure routes through the same `install_net=true` arm, but
/// this time the pre-match history names a uid, so the resolution is
/// `SafetyNetScope::Identity`. Memo `Linux_C2a_FailClosed_Architecture_v2` §1
/// invariant: a known confined identity always gets its one net attempt, so
/// even with the seam armed the install must proceed, never skip.
#[test]
fn a162_slicea_boot_stop_after_resolution_still_installs_identity_scope_net() {
    ...
    assert!(
        ...
        "a known confined identity must still get its one net attempt under a boot-phase stop \
         observed at the refusal decision: {message}"
    );

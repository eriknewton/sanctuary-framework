...
pub struct DaemonConfig {
    ...
    /// TEST-ISOLATION ONLY (A162, LINUX-BOOT-STOP-HOSTWIDE-NET-01): when set,
    ...
    /// AFTER `from_argv` returns (mirroring `test_health_interval_ms`), never
    /// parsed by `from_argv` itself. Compiled out of the shipped binary, so a
    /// production boot has no argv path that can pre-arm this.

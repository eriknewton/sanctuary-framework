...
/// Give the bound socket the group of the runtime directory that contains it.
///
/// Fails closed: a socket whose group could not be set is NOT the socket the
/// deploy doc tells an operator to expect, and returning `Ok` here would ship a
/// control surface that the intended group cannot open while every check still
/// reads green. Failure mode as the operator sees it: the daemon refuses to
/// start with a permissions error naming this path, which is the cheap direction
/// -- the expensive one is a daemon that starts and whose IPC nothing can reach.
fn set_socket_group_from_parent(
    socket_path: &std::path::Path,
    parent: &std::path::Path,
) -> Result<(), IpcServerError> {
    let parent_gid = std::fs::metadata(parent)
        .map_err(|err| IpcServerError::Permissions(format!("socket parent stat failed: {err}")))?
        .gid();
    let socket_gid = std::fs::symlink_metadata(socket_path)
        .map_err(|err| IpcServerError::Permissions(err.to_string()))?
        .gid();
    if socket_gid == parent_gid {
        return Ok(());
    }
    // `None` for the owner leaves the uid untouched; only the group is adopted.
    nix::unistd::chown(socket_path, None, Some(nix::unistd::Gid::from_raw(parent_gid))).map_err(
        |err| {
            IpcServerError::Permissions(format!(
                "socket group could not be set to the runtime directory's group (gid {parent_gid}): {err}"
            ))
        },
    )
}
...
impl IpcServer {
    pub fn start(config: ServerConfig) -> Result<Self, IpcServerError> {
        ...
        std::fs::set_permissions(&config.socket_path, perms)
            .map_err(|err| IpcServerError::Permissions(err.to_string()))?;
        // Mode 0660 only grants anything if the socket's GROUP is the group the
        // deployment provisioned to reach it. A new socket inherits the CREATING
        // PROCESS's egid, which is `sanctuary` only when the daemon was started by
        // the packaged unit (`Group=sanctuary`); started any other way (a manual
        // run, a drill driver, a launcher that drops the supplementary group) the
        // socket lands root:root and 0660 silently grants the group nothing.
        // Adopt the PARENT RUNTIME DIRECTORY's group instead of looking up the
        // name `sanctuary`: the directory is the object the deployment already
        // provisions with the intended group (see
        // `systemd/sanctuary-castle-wall.service`, whose ExecStartPre installs
        // /run/sanctuary/<fortress-id> as root:sanctuary 0750), a name lookup can
        // fail or resolve to a different gid on a host that never provisioned the
        // group, and inheriting the directory keeps a standalone run consistent
        // rather than wrong in a new way. MUST MATCH the expected `stat` output
        // `660 root sanctuary` in `server/docs/castle-wall-linux-deploy.md` and the
        // ownership installed by `systemd/sanctuary-castle-wall.service`.
        set_socket_group_from_parent(&config.socket_path, parent)?;
...
mod tests {
    ...
    fn server_binds_and_unlinks_socket() {
        ...
        // 0660 is only meaningful together with the group it grants. The socket
        // must carry the runtime directory's group, not whatever egid happened to
        // create it; on a deployed host that directory is root:sanctuary, which is
        // what `server/docs/castle-wall-linux-deploy.md` tells the operator to see.
        assert_eq!(
            meta.gid(),
            std::fs::metadata(dir.path()).unwrap().gid(),
            "the IPC socket must adopt the group of its runtime directory"
        );

...
**Under the hood:** the macOS wall is drilled on real hardware, signed and notarized, and survives reboot; the Linux enforcement modules are integration-proven but the shipped daemon does not yet assemble them into live kernel enforcement. Open defect: **IC-02, IC-03, IC-04** (`Review/Sanctuary/Inert_Capability_Sweep_2026-08-07.md`). Cryptographic identity and encrypted state remain operator-held, and exit-bundle portability is partial until the open exit defects above are fixed. It composes with Concordia (agent negotiation) and Verascore (portable reputation), each in its own repo and neither required.
...
|---|---|
| Local `sanctuary protect` (alias `wrap`), dashboard, policy gates, encrypted state, audit trail, exit bundle | Shipped with exit-bundle gaps: dashboard export cannot re-key state, import hides skipped entries, and rotated-key imports can lose pre-rotation state. Open defect: **IC-07, IC-08, IC-09** (`Review/Sanctuary/Inert_Capability_Sweep_2026-08-07.md`) |
...
| Portable identity, state export/import, recovery flows, reputation bundles | Partial for exit bundles until **IC-07, IC-08, IC-09** are fixed (`Review/Sanctuary/Inert_Capability_Sweep_2026-08-07.md`) |

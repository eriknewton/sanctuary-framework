...
- **Sovereign Data Warehouse vault export and import are on the shipped MCP catalog.** `sdw_export` and `sdw_import` were built and tested but never registered by the production server, so no shipped surface could export the vault despite the project rule that persisted agent output be inspectable and exportable. Both are registered by the composition root as non-relaxable Tier 1 (a hand-authored policy listing them under Tier 3 is corrected at load and classified Tier 1 at the gate). Export writes a signed, still-encrypted bundle into a fresh per-export directory under the fortress's own `sdw-exports/` (an operator-directed archive, never a carriage path; a symlink at that path, planted or swapped in, fails the export closed). Bounds: import is all-or-nothing only on a backend with a storage transaction, and the shipped filesystem fortress has none, so `sdw_import` refuses on it today, before any approval prompt; only a bundle this fortress exported (`source_key_ref: this-fortress`) can be re-imported; the raw `identity_sign` tool refuses payloads carrying the SDW export, state-envelope, state-export-bundle, audit-event and receipt signing domains. Post-export deletion (`sdw_export_delete`) is NOT shipped: it needs a backend-wide serialization boundary that does not exist yet and returns as its own item.
- **The multi-agent memory isolation guard pins in the fortress.** `sanctuary wrap` writes `SANCTUARY_AGENT_ID` (`<harness>:<fortress-id>`) into the harness's `sanctuary` MCP entry, and the server keeps the owner pin as a MAC'd record in the fortress, created with an exclusive create on first touch and checked on every memory, provenance, memory-file and vault call, so a second server process over the same fortress with a different wrap-time identity is refused and audited (two independent server graphs over one on-disk fortress, and a simultaneous first touch with exactly one winner, are both covered by tests). A pin removed from a fortress whose memory store is already in use refuses every caller rather than re-pinning; the operator recovery verb for that state is not in this release. Harnesses wrapped before this release carry no identity and pin the scope as unclaimed: re-run `sanctuary wrap` on each harness FIRST, then use the memory tools, and the first identified harness to call them claims the scope once. `sanctuary doctor` warns for each harness entry (JSON configs and the Hermes `config.yaml`) that lacks the variable, for an unclaimed pin when identified entries exist, and for a pin that matches no wrapped entry. Master rotation restamps the pin under the new master (covered by the rotation suite). Bound: the identity is plaintext in the harness-controlled config and accepted unauthenticated, so the guard separates distinct wrapped identities over one fortress; it does not defend against an agent that can rewrite its own harness config and present the first-pinned identity (the same trust basis as the passphrase in that entry). This corrects the 1.7.0 "Multi-agent SDW isolation guard" correction note below.

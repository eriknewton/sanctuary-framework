...
- **Sovereign Data Warehouse vault export, import and post-export delete are now on the shipped MCP catalog.** `sdw_export`, `sdw_import` and `sdw_export_delete` were built and tested but never registered by the production server, so no shipped surface could export the vault despite the project rule that persisted agent output be inspectable, exportable and deletable. They are registered by the composition root and are non-relaxable Tier 1 (a hand-authored policy listing them under Tier 3 is corrected at load and classified Tier 1 at the gate). Export writes a signed, still-encrypted bundle under the fortress's own `sdw-exports/` directory (an operator-directed archive, never a carriage path; a symlink planted at that path fails the export closed). Bounds: the shipped filesystem fortress has no transaction primitive, so on that backend `sdw_import` and `sdw_export_delete` refuse today rather than apply anything partially; on a transactional backend both are all-or-nothing inside one storage transaction; only a bundle this fortress exported (`source_key_ref: this-fortress`) can be re-imported; a delete manifest must match an export this fortress's audit chain recorded, and the raw `identity_sign` tool refuses payloads carrying an internal signing domain.
- **The multi-agent memory isolation guard pins in the fortress.** `sanctuary wrap` writes `SANCTUARY_AGENT_ID` (`<harness>:<fortress-id>`) into the harness's `sanctuary` MCP entry, and the server keeps the owner pin as a MAC'd record in the fortress, checked on every memory, provenance, memory-file and vault call, so a second server process over the same fortress with a different wrap-time identity is refused and audited (proven with two independent server graphs over one on-disk fortress). Bound: the identity is plaintext in the harness-controlled config and accepted unauthenticated, so the guard separates distinct wrapped identities; it does not defend against an agent that can rewrite its own harness config and present the first-pinned identity (the same trust basis as the passphrase in that entry). Harnesses wrapped before this release carry no identity: re-run `sanctuary wrap` on each harness; `sanctuary doctor` warns for each entry that lacks the variable. This corrects the 1.7.0 "Multi-agent SDW isolation guard" correction note below.

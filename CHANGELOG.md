...
- Linux only. The install and upgrade path now has two complete routes with a fixed order, and it asks the new daemon binary whether this host will admit the installed manifest before anything is stopped or replaced. Route one: preflight with the new binary, stop the unit, disarm with the old binary, replace the binary, start. Route two: preflight with the new binary, stop the unit, replace the binary, reboot, which is that route's start boundary and the reason it has no start step. **Upgrade bound:** copying a new binary over the installed path and restarting skips the preflight and the disarm, and the wall's Linux net then covers every uid on the host rather than the agent's own, until the next reboot. `sanctuary doctor` on Linux reports what the daemon's preflight verb answers for the installed manifest, verbatim. Linux Castle Wall enforcement remains `not_implemented` publicly.
- Linux only. `sanctuary castle-wall upgrade-linux --route disarm|reboot --candidate ABSOLUTE_PATH` now exposes those fixed routes to an already provisioned root service from a trusted installed CLI. It derives the fortress identity from the fixed root-owned systemd EnvironmentFile, checks the effective unit, stages and preflights the candidate before stopping the service, then aborts on a failed step with manual recovery information. The operator must invoke a root-owned, non-writable installed CLI/package and Node interpreter under a sanitized environment; the command does not provide first installation. A successful reboot command reports a request, not a completed boot or healthy daemon. Linux enforcement remains `not_implemented` publicly.
...
- **Rung 1 fresh-host custody is recovery-first and fail-closed.** Exact-fortress stored credentials support hands-free local memory reads after restart, and copied fortresses can enroll a new host through the hidden recovery-key ceremony without replacing the master. The reset menu is renumbered recovery-first (recovery-key, then shares, then guardian) with destructive nuke last. Plaintext memory ingest, emit, transcode, transcode restore, and memory/archive export and import remain behind non-relaxable local-human Tier-1 approval. Rung 1 ships as a candidate: before relying on a host for daily use, confirm restart persistence on that host: store a memory, restart, and read it back with no secret typed.
  - The master write barrier degrades to read-only, instead of throwing, when its per-invoker coordinates cannot be resolved for an environmental reason: a non-owner invoking uid (a root daemon or a `sudo`-run verb on an operator fortress), a network/FUSE/exFAT or other unsupported filesystem, or looser-than-0700 `_meta` permissions. An existing fortress on one of those hosts keeps opening for reads; its first master-derived write fails closed with the cause's remediation instead of the process refusing to boot at all.
  - The MCP server now resolves the exact-fortress stored credential (OS keyring, or custody key) read-only at boot when neither `SANCTUARY_PASSPHRASE` nor `SANCTUARY_RECOVERY_KEY` is set, so a host whose passphrase `protect` already stored can boot hands-free; removing the stored credential still fails the boot closed rather than minting a fresh master over existing state. The same exact-fortress stored-credential fallback (argv/stdin/env credentials tried first, then the OS keyring) now backs every memory verb and never generates a credential.
  - `sanctuary reset-passphrase --mode recovery-key` gained `--allow-machine-local-passphrase`: an explicit opt-in to store the freshly enrolled passphrase in the machine-local encrypted fallback file when the host has no OS keyring. Off by default, the rekey fails closed rather than land a passphrase in a machine-bound file with no operator-held copy; opting in prints a loud downgrade warning, and the recovery key remains the durable custody factor either way.
  - An interrupted `reset-passphrase --mode recovery-key` leaves an authenticated rekey journal in place. Master rotation now refuses while that journal exists, naming the exact heal step (re-run the same recovery-key rekey, which resumes and clears the journal) instead of the generic unrecognized-record refusal.
  - A legacy encrypted passphrase fallback file (the prior dirname-derived AAD format) is still read; on the next authenticated success it is upgraded in place to the current identity-derived AAD format.
  - `castle-wall provision-pin` no longer mints a passphrase. It now reads an existing stored credential only, and refuses with a named remedy when none is found, rather than generating custody as a side effect of pinning.
  - `restore-attest` gained `--recovery-key-prompt`: attest with the human-held recovery key, read from a hidden interactive-terminal prompt, instead of the passphrase, for the second-host / lost-passphrase case. It requires a real terminal and never accepts the key from argv, env, or a pipe.
  - On Windows, every memory verb and memory/archive export or import fails closed with a remediation-bearing error ("a local OS approval dialog is unavailable on this platform") before the custody envelope is ever opened, because the reviewed local-human Tier-1 approval dialog is macOS-only. Windows remains scoped to authenticated custody-envelope read only.

- **A fortress that re-exports imported reputation now carries forward a signed record of the keys it verified, so a third fortress can still verify it.** Before this change, a fortress that imported another fortress's reputation and then re-exported (a second hop) had no way to prove those foreign attestations' signer keys, so a receiving fortress reported them unverifiable and refused import even though every hop had individually verified. Exports now include a signed `known_signers` table under a new, additive manifest version that always carries it; the original manifest version never does, so an existing bundle verifies unchanged and an older verifier refuses a new export cleanly through its own existing unknown-version handling rather than misreading it. The table only ever states that a given key derives a given DID: it is never treated as proof that a signer was independently verified by anyone but the fortress that is currently vouching for it, and a table that cannot be trusted (malformed, incorrectly signed, or naming a signer it should never name) fails the whole verification and import outright rather than degrading silently. A signer key retired for a disclosed-compromise reason is never carried forward by this table without the same explicit operator opt-in the rest of exit already requires for compromised keys, and the table's own storage is capped rather than allowed to grow without bound.
- **Sovereign Data Warehouse vault export and import are on the shipped MCP catalog.** `sdw_export` and `sdw_import` were built and tested but never registered by the production server, so no shipped surface could export the vault despite the project rule that persisted agent output be inspectable and exportable. Both are registered by the composition root as non-relaxable Tier 1 (a hand-authored policy listing them under Tier 3 is corrected at load and classified Tier 1 at the gate). Export writes a signed, still-encrypted bundle into a fresh per-export directory under the fortress's own `sdw-exports/` (an operator-directed archive, never a carriage path). Containment bound: the write refuses a symlink anywhere on the validated path, rechecks the directory's identity immediately before the file is opened and immediately after it is renamed, and unlinks what it wrote on any change; a swap that lands inside the window between a recheck and the next filesystem call is not detected (Node exposes no directory-relative open or rename). Import is all-or-nothing only on a backend with a storage transaction, and the shipped filesystem fortress has none, so `sdw_import` refuses on it today, before any approval prompt; only a bundle this fortress exported (`source_key_ref: this-fortress`) can be re-imported; the raw `identity_sign` tool refuses payloads carrying the SDW export manifest, state-envelope, audit-event, receipt and audit-checkpoint signing domains. Post-export deletion (`sdw_export_delete`) is NOT shipped: it needs a backend-wide serialization boundary that does not exist yet.
- **IC-16: one wrapped agent now owns a fortress's SDW memory across server processes.** The first identified wrapped agent to touch a fresh, empty SDW atomically creates a master-MAC-authenticated owner pin; every memory, memory-file, provenance, migration, signer-maintenance, export, and import route authenticates that same durable pin before touching the shared scope. A second process with another wrapped identity is refused on its first call. Filesystem first-claim uses an atomic no-overwrite publish, and owner transfer uses a genuine lock-serialized compare-and-replace rather than the unsafe read-compare-rename prototype. Existing unpinned SDW data is never inherited by whichever process arrives first: it refuses until the operator stops Sanctuary and runs the interactive `sanctuary sdw-owner claim`; transfers are also interactive, name the exact current and new identities, and compare-and-replace the exact authenticated record. `sanctuary doctor` reports missing, invalid, mismatched, and locally-unmatched pins. The cooperative-mode identity remains the wrap-written harness configuration value; this closes distinct correctly wrapped cross-process isolation, not an agent that can rewrite and relaunch another harness's entire MCP configuration.
- **`sdw_memory_provenance` reports passages admitted by a memory archive import.** A passage that arrived through a cross-fortress memory archive import now reports `verified` with its origin-trust tier (foreign for a signed-memory archive, legacy-unattested for a V1 archive) and the transfer lineage reference named by the import that admitted it. The tool had returned a generic denial for every such passage while `memory_get` read it back and the underlying provenance verified. The tool binds the foreign origin to the local passage through the admission record this fortress signed at import time (the origin digest it commits to, the local passage id it names, and the lineage reference it carries); the passage-id check is not loosened, and a companion presented for a passage it does not describe is still refused. Locally authored passages report exactly what they reported before.
...
The v1.7.0 tag was cut before three gate-path fixes landed. As tagged, an unmodified agent harness could not reach the exclusive-egress gate (reaching it required a bespoke token header that no stock HTTP client sends), and a gate-daemon plist healed at boot did not take effect until the NEXT boot, because launchd had already loaded the stale job. v1.7.1 carries the reachability fix (#1024) and both boot fixes (#1027, #994), so the gate path is usable by stock proxy-honoring agents and a healed plist takes effect in the boot that healed it. Claim bound: the bootout-before-bootstrap ordering and the combined heal-then-take-effect sequence are unit-proven against the launchd seams; the hardware reboot re-drill of the combined behavior is still pending, so the same-boot self-heal is not claimed proven on real hardware yet.

### Fixed

- **Exclusive-egress gate is reachable by proxy-env-honoring harnesses.** The gate now accepts standard HTTP Basic proxy credentials whose password carries the existing generation-bound token payload, and the release wrapper exports HTTP(S)_PROXY/http(s)_proxy only at exec time after the release barrier and token checks pass. Clients that ignore proxy environment variables still cannot use this path; this does not add transparent interception (#1024).
- **Exclusive-egress proxy credential residual is documented.** The wrapper delivers the per-generation proxy credential through the agent harness environment, so agent-side tooling that dumps its own environment can expose it in harness logs; credentials rotate per generation and remain uid-bound at the gate. `NO_PROXY` remains unset because pf confinement already blocks direct loopback for the confined uid; setting it would change the denial path, not restore loopback introspection (#1024).
- **Boot path boots out a stale loaded gate-daemon job before bootstrapping, so a healed plist can take effect in the SAME boot.** A 12-cold-reboot hardware drill showed the plist self-heal writing the healed file to disk on every stale-plist boot (5/5) while the gate daemon still started on none of them (0/5): launchd had already loaded the stale job, so a raw `bootstrap` no-opped and `kickstart` relaunched the cached stale argv. Both the gate and the sibling peer-resolver boot paths now route through the shared bootout, settle-to-unloaded, bootstrap, kickstart chokepoint; an already-running healthy daemon is pre-checked and left alone; and a gate-daemon bootstrap failure is a HARD parked outcome folded into the release-barrier verify, never a logged-and-released one (#1027).
- **Boot self-heals a stale pre-#986 gate-daemon plist.** A host armed on a pre-#986 build carries a gate-daemon plist with a non-absolute `node` interpreter that fails on every boot and previously required a manual repair. The boot supervisor now re-renders the plist with the boot daemon's own absolute interpreter path through the same builder the install path uses, atomically and fail-safe: a write failure is logged and audited, and the release barrier still parks fail-closed (#994). Combined with #1027 the heal is intended to take effect in the same boot; that combined behavior is unit-proven, with the hardware re-drill pending.
- **Observe/learn promotion now reaches live enforcement (#1031).** Promote publishes rules into the same `policy/egress/rules/` source the enforcement daemons read (the ratified Option A convention), scoped to the current routing mode (gate-uid under exclusive routing, with rewrite on mode change and an operator-gated rescope path for orphaned rules), serialized against provisioning by a shared cross-process writer lock, and guaranteed to carry exactly one genuine habeas distress lane so a fresh promote composes on Linux. Five review rounds with an independent second-family gate at each; the on-hardware promote-then-reach drill (an agent-side CONNECT through the gate after re-arm) is still owed before any reach claim widens.
- **Honest `promote` copy: promoted observe/learn rules do not yet reach live enforcement.** The promote success message and help text claimed promoted rules take effect immediately, but promote currently writes to a rule source the enforcement daemons never read, so approved destinations stay denied (fail-closed). The CLI copy and the ROADMAP entry now state that defect plainly until the enforcement wiring lands (#1030).
- **Release-barrier refusals are observable.** A refused release records why, so a parked agent is diagnosable instead of silently held (#1025).
- **Exclusive-egress oracle refresh now fails closed loudly on hung refreshes.** The root supervisor bounds a refresh attempt, invalidates known tokens at timeout, caps abandoned work, and guards the oracle write path so a late abandoned attempt cannot mint a live token. A permanently abandoned probe can still cap later attempts; it now surfaces as stuck and denies by token expiry instead of silently preserving freshness (#1023).
- **Harness park verification waits for launchd to settle.** Repair/unprotect park verification reuses the shared stopped-settle loop and refuses fail-closed with measured elapsed time and sample count when the harness remains running or launchd status is untrustworthy. The `--stand-down-agent` flag is now required to acknowledge that repair and unprotect stop/disable the agent harness (#1023).
- **Castle Wall daemon collision messages show a real pid when available.** The already-running message interpolates the active-config pid, or says `pid unavailable` when the socket-only guard has no pid, and no longer emits the literal `<pid>` placeholder (#1023).
- **Mesh split-brain is raised on a policy refused as a replay, not only on one applied** (#1022).
- **One exported chokepoint for wrap's PyYAML interpreter resolution.** Refusals name what was probed, and a doctor check shares the same matcher so the check cannot drift from the behavior it checks (#1020).

### Release pipeline

- **Notary credentials are validated against Apple in preflight, before any build step** (#1018).
- **The publish signing gate fails closed.** The step whose name falsely reported signing as skipped is deleted, and a fail-closed probe runs before the irreversible `npm publish` (#1019).

### CI

- Playwright container image is derived from `server/package.json` instead of hand-pinned (#1029); two timing flakes are fixed at the cause, not with bigger timeouts (#1021); GitHub Actions bumps: checkout 7 (#970), setup-node 7 (#971), setup-python 7 (#1026), rust-toolchain (#972).

### Dependencies

- Server: minor-patch group, 7 updates (#1017), body-parser (#985), hono (#1009), fast-uri (#1001), brace-expansion (#982), postcss (#1011). Menubar: serde_with (#941), quinn-proto (#1010).

## [1.7.0] - 2026-07-26

Minor release. Roughly three and a half weeks of feature, hardening, and fix work since v1.6.1 (2026-07-01). The additions are backward-compatible and the new enforcement mechanisms fail closed. This section groups the work; per-PR detail is in the git history from the v1.6.1 tag through the v1.7.0 tag.

Two headlines. First, this is the first release whose macOS app artifact is signed and notarized: the release pipeline names a missing Apple credential instead of dying inside Keychain (#1008) and notarizes before zipping (#1015). Second, the unified-protect exclusive-egress path lands end to end in code: one protect flow can provision a confined agent account, arm a per-uid pf-anchored exclusive route through a dedicated gate process, hold the agent parked behind a release barrier until the gate verifies, and tear down per agent through a registry.

Known limit, fixed in v1.7.1: this tag predates the gate-reachability fix (#1024) and the boot fixes (#1027, #994). As tagged, an unmodified agent harness cannot reach the exclusive gate, and a gate-daemon plist healed at boot takes effect only on the next boot. See [1.7.1].

### Added

- **Exclusive-egress gate (unified-protect Slice 5).** A locked multi-uid pf-anchor registry (#935), a gate-port generation state machine with a bind-first helper (#938), the gate as its own trusted computing base with a dedicated `sanctuary-gate` uid, fail-closed client auth, and a root liveness oracle (#943), exclusive routing composition with gate-lane byte-identity (#948), a release barrier that holds the agent parked behind a parked LaunchDaemon, a root hold file, and exec-wrapper verification (#957), arming wired into the provision flow (#959), per-agent unprotect through the registry with a sole-agent invariant and an all-verb provision lock (#961), an exclusive-egress posture object rendered coarse-only on every surface (#947), and the unified-protect enforcement core underneath it: loopback gate, pf confinement, peer-id, policy parity (#854).
- **Castle Wall confined-agent provisioning and observe/learn.** Signed per-uid allow-list egress provisioning that refuses to arm fail-closed (#905), agent-account auto-provisioning folded into one protect flow (#893, #901, #903), configure-origin folded into enable for a one-command arm (#892), observe/learn allow-list v1 with deny-and-record and a human-gated promote (#897), and two-confined-uid classification with a rule-scope axis (#930).
- **Castle Wall audit and export rail.** Slice M macOS audit-producer authenticity: producer-key mint and publish, full-mode daemon signing, and an `audit-verify` CLI (#927), a signing watchdog that makes a silent per-flow audit-emission stall loud (#946), an emission-liveness watchdog detecting decided-versus-emitted divergence (#960), an operator-side enforcement-event exporter with a frozen public event schema (#907), and a streaming consumer with a durable cursor, config loader, and CLI verb (#911).
- **File grant.** Box-local read-only governed file-grant v1 (#896), a cross-uid read-scope enforcement primitive built on an inode-scoped ACL plus an agent-uid readability probe (#917), and reachability plus fortress-traverse boundary hardening from drill F1 (#925).
- **Evidence pack v0.1-preview.** Slices 1 through 4, honesty-swept (#933), then hardened by repeated honesty-chokepoint waves: daemon-store disclosure at every census-presenting render site (#939), egress-legend re-truing (#937), dry-bar close-outs (#942), honest merged-census arithmetic (#953), retention determinability (#954), usable-figures honesty (#958), census/coverage consistency and manifest serialization (#962), attested-window counting and read-failure sanitization (#966), and unjustified-claim chokepoints (#969).
- **Fleet and entitlement.** Operator add-machine enrollment with node-count capacity binding (#894), signed compliance-attestation export with offline verify (#895), an offline Ed25519 fleet-entitlement scaffold that fails closed to community (#859), paid-tier license issuance CLI with a v2 signed schema and tamper-evident ledger (#868), ledger anti-rollback anchor and issuer-key pin (#871), downgrade UX with a signed revocation list and an operator-visible downgrade log (#890), node-count enforced on the daemon federation roster (fleet control plane PR-B), and a persisted, rehydrated node roster (#888).
- **Dashboard and UI.** One paper/ink token system shared across every web surface (#963), a posture-first landing with nav reorder (#964), an evidence spine on existing data: denominators, freshness, evidence links (#965), unified login and fleet switcher on the same design (#850), a read-only fail-closed fleet-roster panel (#861), and slice 1 of a PWA mobile companion, phone as an approval surface (#992).
- **Privacy and query anonymity.** Tier B PII rewrite wired into the live substrate path (#996), the privacy-filter tier-2 surface pinned local-only (#997), and zero unrequested outbound by default with the update check made opt-in (#869).
- **Substrate and wrap.** A second bundled blocklist plugin with a registry-pinned-key loader chokepoint (#865), a Hermes-YAML parse-parity guard that refuses if the sidecar is down (#864), and surface-scoped wrap-meta and backups with non-destructive meta removal and an orphan-guard on all rollbacks (#848).
- **Docs and positioning.** North-star positioning applied across the public surface (#998, #1000), machine-resident-factor custody-bootstrap posture (#993), the "Residency Is Not Custody" blog piece (#923), a habeas honest-limit section (#874), a standards compliance-evidence crosswalk (#876, #878), and a configure-origin step in the macOS arm runbook (#877).

### Security

- **Audit stores split by writer.** The fortress audit store is split into a root daemon chain and an operator chain (#929), extended to the checkpoint namespace (#945), with atomic lock acquire and inode-verified staleness break (#944), inode-verified lock release, clock-skew-safe staleness, and temp GC (#950), a reload-drop chain marker with an O(1) in-lock tail scan (#922), and an authenticated, head-clamped enforcement-export cursor (#952).
- **Evidence binding.** Every protection claim is bound to the subject of its evidence (#976), digest subjects are bound to posture evidence (#978), audit attribution is verified before projecting agents (#979), and Linux canonical subjects are bound with the operator baseline mirrored in the Rust verifier (#980, #981).
- **Cross-language canonicalization parity.** The sysext manifest canonicalizer is byte-parity with the Node signer, closing a drill-found silent reject of every egress manifest (#915), unsafe-integer canonicalization is rejected fail-closed (#924), uncanonicalizable audit events settle as rejections rather than looping (#926), and adversarial cross-language parity vectors pin the contract (#921).
- **Daemon robustness.** Egress policy reload can no longer hang the daemon (#912), derived DNS allow is scoped to the host's active resolver set (#914), the daemon heartbeat no longer clobbers the operator TTL so the dead-man auto-disarm fires (#885, contract pinned in #887), enable refuses without an agent-origin descriptor (#883, #884), a signer-helper boot-readiness preflight runs before arming (#882), and launchctl calls are bounded with no destructive same-fortress rebootstrap (#904).
- **Federation and recovery hardening.** Fail-closed guardian Recovery Cascade covering quorum-reuse, envelope-confusion, and malformed rosters (#858), atomic guardian-requirement mutation with a deleted-sync-state fail-closed latch (#856), an external anti-rollback anchor and audit-witnessed floor for the guardian disable-gate (#867), forged reputation imports rejected with aggregates-only and tier-weight tamper-proofing (#862), fail-closed workload host-attestation and undeclared-finding seal origin-auth (#863), and attestation badge green positively earned with no false-green on signal drop (#866).
- **Surface hardening.** An initial-url SSRF gate in httpGetText with strict base64url manifest decode (#870), console 500 responses no longer leak internal error detail (#872), and wrapped-install upgrade advice carries a semver-shape guard (#873).
- **Structural guards.** Template literals are tokenized in comment stripping so the em-dash and attribution guards cannot be fooled (#1004), and the test-baseline guard scopes its fail-above demand to server/ commits and installs where git actually runs the hook (#967).

### Fixed

- **The exclusive-egress drill wave.** Drill-found fixes on the install-and-arm path: native dscl attribute-prefix parsing (#977), hold-dir chokepoint and parked-install stand-down (#968), stale exclusive-routing-marker self-heal and gate-daemon crash surfacing on arm timeout (#983), an absolute node interpreter pinned in the gate-daemon argv (#986), Hermes config-parity python resolved by PyYAML importability (#987), the oracle refresh loop starting, and loudly reporting when it cannot start, on an empty boot registry (#988, #991), confined-agent peer uid resolved via a privileged resolver (#1003), a confined Hermes that can start, re-arm, and restore the agent's grants on a refused arm (#1006), a mode-independent pre-mutation residue gate (#1012), the pf enable reference bound to a boot session and to our own anchor (#1013), and the fortress named in every passphrase resolution, with a test failing closed when it resolves the operator's own (#1014).
- **Castle Wall accuracy.** Honest drain-fault basis and heartbeat/producer-signing comment accuracy (#984), watermarked exactly-once observe-fold idempotency (#931), Linux daemon FLAT-shape egress folds (#934), promote preserving signed operator-baseline and agent-origin descriptors (#899), the daemon subcommand honoring `--fortress` (#900), and provision-pin no longer clobbering the signer-owned global pin (#898).
- **Install and tooling.** Quickstart clean-install build unbroken (#936), the vite-node script runner pinned, removing a phantom dependency (#949), staging-record validation with fleet-deps dedup (#956), and test de-flakes across castle-wall, dashboard, and federation suites (#891, #920, #928, #989).

### Dependencies

- Grouped minor-patch bumps across server, quickstart, and menubar (#844, #845, #879, #913).
...
- **Optional M-of-N guardian sign-off on the revoke and kill path.** A default-off, fail-closed gate that can require a threshold of guardian signatures before a federation revoke or kill action proceeds (#824). Correction, 2026-08-07: this quorum gate cannot currently be enabled on a shipped fortress; the single-operator revoke path is what runs. Do not rely on guardian quorum for revoke or kill. Open defect: **IC-10**.
...
- **Competitor-readiness hardening wave.** A fail-closed signed-release-manifest verifier for the self-update advisory (#818): the verifier mechanism is built and rejects any unsigned, wrong-key, or tampered manifest, but it is NOT yet wired into the live update path and ships behind a placeholder key, so the live update advisory is not yet signature-gated (the signed path stays disabled until a real release key is pinned). Plus a config-downgrade gate rebuilt on a custody-MAC anchor, replacing an earlier unsigned baseline (#805); mesh anti-rollback and activator-downgrade gates (#804); and a config-baseline deletion-replay close-out anchored to a boot-anchored monotonic witness (#817, gated HALT pending custody-core review).
...
- **Multi-agent SDW isolation guard.** A guard intended to block multi-agent Sovereign Data Workspace memory use until per-agent custody isolation lands (#822). Correction, 2026-08-07: the original process-local guard did not hold across two agents sharing one host. Correction, 2026-09-01: the Unreleased durable one-owner-per-fortress binding closes that distinct correctly wrapped cross-process case; the cooperative harness-config rewrite bound remains explicit.
...
No functional change since rc.1. Phase 1 Linux and macOS enforcement surfaces remain the production baseline. (Correction, 2026-08-07: only the macOS surface enforces. The shipped Linux daemon installs no kernel enforcement. Open defect: **IC-02, IC-03, IC-04**.)
...
- **Castle Wall Linux Phase 1 remains the production enforcement baseline.** The Linux packet-filtering path is the current real Layer 1 enforcement surface for wrapped agents. (Correction, 2026-08-07: it never was. The shipped daemon installs no nftables table, binds no NFQUEUE, creates no cgroup scope, and never calls the deny-by-default evaluator. Open defect: **IC-02, IC-03, IC-04**.)
...
### v1.x, WP-V1.x-QUERY-LAYER-ANONYMITY STRUCTURALLY COMPLETE

- **Rho-3 WP-V1.x-QUERY-LAYER-ANONYMITY Tier B smart mode (CLOSES Principle 4 (Opacity at the query layer) end-to-end with Tier A plus Tier B both shipped).** Rho-1 closed Principle 4 at the foundation level via Tier A header strip default-on. Rho-2 added Tier B basic PII rewrite. Rho-3 lands Tier B smart mode and structurally completes the work package. Smart mode adds three capabilities. (1) Intent classifier at `server/src/query-anonymity/intent-classifier.ts` invokes `SubstrateSelector.invokeClassify()` on the existing `privacy-filter-tier-2` surface to decide which PII classes a given query intent requires preserved versus anonymized; classifier failure or low confidence falls back to anonymize-all so the privacy-preserving choice is also the failure-safe choice. (2) Smart-mode rewriter at `server/src/query-anonymity/smart-rewriter.ts` combines Rho-2 PII detection with intent-based class preservation; placeholders are emitted with deterministic shape so concierge can map them back at render time. (3) Per-query encrypted reverse-mapping memo at `server/src/query-anonymity/reverse-mapping-store.ts` uses HKDF plus AES-GCM with AAD bound to `fortressPath` plus `query_id`, so the operator sees responses with original terms restored even though the substrate never saw the originals; if the reverse map cannot be read, the concierge surface fails soft with an inline notice instead of leaking placeholders. `smart_mode_enabled` is added to the existing Tier B `PiiConfigStore` and implies effective Tier B while keeping the basic toggle independently operator-visible. Four Rho-3 audit events ship: `query_anonymity_intent_classified`, `query_anonymity_smart_rewrite_applied`, `query_anonymity_smart_mode_fallback`, `query_anonymity_reverse_mapping_used`. 18 new smart-mode regression tests. Why this matters: per the Sovereignty Stack Assessment 2026-05-10, no comparator surveyed ships query-layer anonymity at strength; smart mode (with operator-side reverse mapping so the UX does not degrade) is the visible end of that genuinely novel surface. PR #215.

### v1.3 preview, WP-V1.3-6 English-Authored Policy Gates (2 of N to 3 of N)

- **Xi-3 WP-V1.3-6 policy conflict detection.** Builds on Xi-1 compile-then-activate foundation and Xi-2 activation lifecycle. Xi-3 adds pre-activation gating that surfaces conflicts between compiled policy artifacts before they take effect, so operators cannot quietly activate a policy whose rules contradict another already-active policy. Conflict detection runs structurally at activation time, separate from authoring, preserving the additive-only invariant on the existing policy surface; operator decides whether to resolve, override, or rescind. PR #216.

### v1.3 preview, WP-V1.3-7 Auto-Trigger Ladder + Threshold Calibration (2 of N to 3 of N)

- **Nu-3 WP-V1.3-7 auto-promotion criteria plus calibration recommendations.** Builds on Nu-1 foundation (threshold configuration store, 3-rung action ladder, encrypted at-rest store with AAD bound per fortress). Nu-3 adds a 60-second scheduled `CalibrationSuggester.tick()` that evaluates rule history and emits `auto_trigger_recommendation_generated` events with a reject cool-down. Load-bearing invariant preserved structurally and tested: `tick()` never calls `promoteRule()` or `demoteRule()`; the only path that changes a rung is `acceptRecommendation()`, reached through explicit operator routes or CLI accept. Promotion is operator-driven, never autonomous, even when the recommendation is high-confidence. PR #217.

### v1.3 preview, WP-V1.3-5 Honeypot Authoring (2 of N to 3 of N)

- **Pi-3 WP-V1.3-5 tool-call honeypot class.** Builds on Pi-1 foundation (typed trap definitions, deterministic compilation, fortress-local storage) and Pi-2 filesystem trap class plus fortress-config persistence. Pi-3 adds the third concrete trap class, tool-call, completing the http_endpoint plus filesystem plus tool_call trio so the deception surface covers the three behavior shapes a compromised agent is most likely to exhibit when probing for valuable state. Authoring stays separate from enforcement (Pi-1 invariant preserved); trap class is additive to the existing Honeypot Authoring surface and respects the no-outbound-by-default rule. PR #214.

### v1.x, WP-V1.x-RECOGNITION-LAYER Path C build 4: key rotation

- **did:web Path C build 4: key rotation.** Build 1 (#190 in v1.2.11) shipped issuance, resolution, and publication scaffold; build 2 (#203 in v1.2.14) wired did:web through exit bundles; build 3 (#210 in v1.2.15) made fortress-config inclusion automatic. Build 4 advances the production-credibility surface by adding key rotation, so an operator who needs to rotate a did:web identity key can do so without breaking the recognition surface mid-flight. did:web continues to operate without trusting Sanctuary as an intermediary; rotation flows through DNS plus TLS plus HTTPS at the operator's authority host, and the receiving regime verifies the new key the same way it verified the original. PR #218.

### Notes

Test baseline floor: 4368, reflecting the iteration-14 cascade over the v1.2.15 floor of 4249. Two PRs in this cascade documented cascade-math overcount in their bodies as a transparency note rather than a regression. Rho-3 projected delta against base 4249 in isolation was +68 (4317); on top of merged Pi-3 plus Nu-3 plus Xi-3 (floor 4302), actual passing count was 4347, so the Rho-3 floor was set to 4347 to match observed reality (+23 gap reflects overlapping new-test surfaces, not test regression). did:web build 4 projected delta against 4249 in isolation was +71 (4320); on top of merged Pi-3 plus Nu-3 plus Xi-3 plus Rho-3 (floor 4347), projected floor was 4418 but actual passing count was 4368, so the floor was set to 4368 (+50 gap reflects the same cascade-math overcount class). No test regressions; full suite passes clean at 4368 on the v1.2.16 release commit. npm latest before this cut is whatever Erik most recently published; v1.2.16 is queued behind any earlier patches awaiting OTP and publish remains Erik-owned because OTP is required.
...
- **v1.1 routes absent on wrap-emitted dashboard (Finding V).** v1.1.1 mounted v1.1 dispatch on `principal-policy/dashboard.ts` (the standalone and MCP-server-boot dashboard) but missed the third dashboard caller. `cocoon/cli.ts:runWrap` starts `dashboard/server.ts` (the legacy operator dashboard the wrap-emitted URL hits). Operators following the documented wrap flow saw `/v1.1`, `/api/hub/*`, and `/api/identities` return 404 against the published binary. The hotfix extracts a shared dispatch helper at `server/src/dashboard/v1_1/dispatch.ts`, plumbs it into both dashboard servers, and wires the v1.1 bindings on the wrap-auto dashboard with a real master-key-derived `AuditLog` so the activity feed reads from the actual audit log instead of a stub.
...
- `publishV11Event` SSE producer extension is queued in v1.1.x housekeeping. Hub events do not yet fan out to `/api/stream` consumers in real time. Correction, 2026-08-07: this entry said "the dashboard polls instead", and nothing polls. `publishV11Event` does not exist in the tree, the declared `inbox` and `agent_status` SSE events are never broadcast, and the polling fallback is gated on an `EventSource` construction failure that no shipped path triggers. The dashboard shows stale data until the operator reloads. Open defect: **IC-12**.
...
- **Fortress-level Tier 1 endpoints** (PR #77): `POST /api/hub/fortress/lockdown` and `POST /api/hub/fortress/exit-bundle/export`. Each creates a single inbox item; on operator approval the handler iterates per-agent (lockdown) or runs `exportExitBundle()` once at fortress scope (export). Routing-layer sentinel alias maps the dashboard's `/api/hub/agents/all/lockdown` and `/api/hub/agents/all/exit-bundle/export` call paths to the canonical `/fortress/*` handlers. Closes Dashboard Backend Binding Addendum §8.1. Correction, 2026-08-07: fortress lockdown does not currently gate any write, egress, or tool call, and its status surface is display-only. Do not rely on lockdown as an enforcement control. Open defect: **IC-11**.

...
- **`sanctuary wrap` writes a per-harness identity, and the memory isolation guard resolves it.** `SANCTUARY_AGENT_ID` (`<harness>:<fortress-id>`) is written into the harness's `sanctuary` MCP entry, and one guard instance covers the memory, memory-file, provenance and vault tools, refusing a second identity that reaches the same server process (the provenance tool refuses it before the approval gate). Bound: the guard separates identities WITHIN ONE SERVER PROCESS only; two harnesses wrapped over one fortress each run their own server process and are NOT separated by it. IC-16 stays open and returns with a cross-process design (a create-if-absent pin plus a true compare-and-swap). Harnesses wrapped before this release carry no identity: re-run `sanctuary wrap` on each harness; `sanctuary doctor` warns for each harness entry (JSON configs and the Hermes `config.yaml`, read through the wrap scanner and scoped to the `sanctuary` entry) that lacks the variable. This narrows, and does not close, the 1.7.0 "Multi-agent SDW isolation guard" correction note below.
...
- **Optional M-of-N guardian sign-off on the revoke and kill path.** A default-off, fail-closed gate that can require a threshold of guardian signatures before a federation revoke or kill action proceeds (#824). Correction, 2026-08-07: the gate cannot be turned on. `setFederationGuardianRevocationRequirement` has no production caller, and no HTTP route, CLI verb, MCP tool, environment variable, or peer-sync ingest reaches it, so every shipped fortress takes the legacy single-operator revoke path and the quorum machinery is reachable only from tests. Open defect: **IC-10**.
...
- **Multi-agent SDW isolation guard.** A guard intended to block multi-agent Sovereign Data Workspace memory use until per-agent custody isolation lands (#822). Correction, 2026-08-07: the guard is wired into `createSdwMemoryTools` but its refusal branch cannot fire. It pins the first observed owner identity and refuses only when a later call observes a different one; that identity resolves from `SANCTUARY_AGENT_ID`, which no shipped path sets in the server's own process environment (`server/src/index.ts:1453` reads `process.env`; the one writer, `buildHarnessEnv()` at `server/src/agent-contract/adapters/tier-b-sdk.ts:235`, builds a child harness env and has test-only callers), so the value is constant for the life of the process. Two wrapped agents on one host still read and write each other's passages through the memory tools. Do not read this entry as shipped isolation. Open defect: **IC-16**.
...
- **Fortress-level Tier 1 endpoints** (PR #77): `POST /api/hub/fortress/lockdown` and `POST /api/hub/fortress/exit-bundle/export`. Each creates a single inbox item; on operator approval the handler iterates per-agent (lockdown) or runs `exportExitBundle()` once at fortress scope (export). Routing-layer sentinel alias maps the dashboard's `/api/hub/agents/all/lockdown` and `/api/hub/agents/all/exit-bundle/export` call paths to the canonical `/fortress/*` handlers. Closes Dashboard Backend Binding Addendum §8.1. Correction, 2026-08-07: the lockdown half of this entry reports success and locks nothing. The wired agent controller throws a typed capability error for every agent, the caller swallows each throw into a `failed` list, and then unconditionally appends a `fortress_lockdown_engaged` audit entry with `locked_count: 0` and persists `active: true` with no check on that list. Every reader of lockdown status is display-only; nothing gates a write, an egress, or a tool call on it, and no shipped path ever lifts it. Open defect: **IC-11**.

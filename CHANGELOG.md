...
- **Sovereign Data Workspace (SDW).** Storage spine with an enforced write gate that cannot persist secrets, Phase 2 stores (working-state, query-history, document-corpus), blind plaintext query-history timestamping, provenance-derived taint propagation, and a memory-backend adapter contract (#420, #421, #449, #440, #438, #484). Current correction: approval-bound vault export/import is not shipped because `createSdwTools` has no production caller, so persisted SDW agent output is not exportable despite AGENTS.md MUST-NEVER #2. Open defect: **IC-15** (`Review/Sanctuary/Inert_Capability_Sweep_2026-08-07.md`).
...
- `publishV11Event` SSE producer extension is queued in v1.1.x housekeeping. Hub events do not yet fan out to `/api/stream` consumers in real time. Correction, 2026-08-07: this entry said "the dashboard polls instead", and nothing polls. `publishV11Event` does not exist in the tree, the declared `inbox` and `agent_status` SSE events are never broadcast, and the polling fallback is gated on an `EventSource` construction failure that no shipped path triggers. The dashboard shows stale data until the operator reloads. Open defect: **IC-12** (`Review/Sanctuary/Inert_Capability_Sweep_2026-08-07.md`).

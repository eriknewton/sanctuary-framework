 ...
 * round to reduce drift. A ratio far outside a prior result needs investigation;
 * contention is one possible cause, not a reason to dismiss a regression.
 */
...
/**
 * The "before" arm, byte-for-byte the funnel body on the RC5 release base
 * (bced40d3934073ae8a4405a38a82aa44de70e739).
 *
 * Must match the `return ed25519.verify(...)` line in
 * `server/src/core/identity.ts` as it stands on bced40d3; `assertBaseline`
 * below fails the run if that commit no longer contains it.
 */
const RELEASE_BASE_FUNNEL_SOURCE = "return ed25519.verify(signature, payload, publicKey);";
const RELEASE_BASE_COMMIT = "bced40d3934073ae8a4405a38a82aa44de70e739";
...
function assertBaseline() {
  const releaseSource = execFileSync(
    "git",
    ["show", `${RELEASE_BASE_COMMIT}:server/src/core/identity.ts`],
    { encoding: "utf8" }
  );
  if (!releaseSource.includes(RELEASE_BASE_FUNNEL_SOURCE)) {
    throw new Error(
      "release base " + RELEASE_BASE_COMMIT + " no longer contains the funnel " +
        "this benchmark calls the baseline; update RELEASE_BASE_FUNNEL_SOURCE " +
        "and permissiveVerify together."
    );
  }
}
...
function main() {
  ...
  process.stdout.write(
    [
      ...
      report("before (bced40d3 release base, permissive)", beforeMedian),

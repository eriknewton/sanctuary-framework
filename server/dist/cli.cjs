...
var init_zk_proofs = __esm({
  "src/l3-disclosure/zk-proofs.ts"() {
    ...
    H = ed25519.RistrettoPoint.hashToCurve(H_INPUT);
    ...
    ClientManager = class {
      ...
      async doConnect(conn) {
        try {
          ...
          const client = new index_js$1.Client(
...
function fallbackFilePath(home) {
  return path.join(home, ".sanctuary", "passphrase.enc");
}
...
async function writeToFallbackFile(path$1, value, home) {
  const dir = path.join(home, ".sanctuary");
...
async function runCompliance(args) {
  ...
  const manifestPath = path.join(outputDir, "00_bundle_manifest.json");
  ...
  for (const file of bundle.files) {
    const path$1 = path.join(outputDir, file.filename);
    await promises.writeFile(path$1, file.content, "utf-8");
    totalManualMarkers += countManualInputMarkers(file.content);
  }
  let pdfPath;
  if (opts.pdf) {
    pdfPath = path.join(outputDir, "bundle.pdf");
    const pdfBytes = renderBundleToPdf(bundle);
    await promises.writeFile(pdfPath, pdfBytes);
  }
  console.error("");
  console.error(

...
// 80 microtask turns: enough for the commit path to reach the held `_audit`
// write. The audit load path grew a few awaits when the checkpoint-signing
// latch landed (IC-05: one latch read per full verify), which pushed the
// held write past the previous 40-turn drain; the count is a harness
// budget, not a property under test.

...
const ALLOWLIST: Record<string, string> = {
  ...
  "src/cli/reset-passphrase.ts": "H1/S2 recovery-key rekey (mode d): re-wraps the SAME master under a fresh passphrase - master, state namespaces, and recovery wrap all preserved, UNLIKE rotateMaster's full re-encryption - so it never touches the state that an interrupted exit-import journal's snapshots are bound to. It manages its OWN durable custody-rekey journal (custody-rekey-journal), not the exit-import journal, and writes only the custody envelope + audit + reset-history, none through the N4-guarded StateStore/ReputationStore write chokepoints. A stale exit-import journal therefore survives unharmed across a passphrase rekey until a wired open path or a `sanctuary exit` verb runs recovery; not wired this fix round, same bounded-scope reason.",
};

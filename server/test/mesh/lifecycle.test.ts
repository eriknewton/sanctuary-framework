    ...
    const eventAt = (policyVersion: number, seq: number) =>
      packSignedEvent({
        ...
        payload: {
          ...
          policy_blob: compiledPolicyBlob("agent-replay", policyVersion, {
            fortressId: first.bootstrap.master_public.fortress_id,
          }),
        },
        monotonic_seq: seq,
        node_private_key: peerKeypair.privateKey,
  ...
  it("delta-applies policies past the requester's baseline", () => {
    ...
    const placeholder: SignedEvent = {
      ...
      payload: {
        ...
        valid_from: "2026-06-01T00:00:00.000Z",
        valid_until: "2099-01-01T00:00:00.000Z",
        policy_blob: compiledPolicyBlob("agent-A", 2),
      },
      payload_hash: "h",
    ...
    const fakeEvent = (v: number) =>
      ({
        ...
        payload: {
          ...
          valid_from: "2026-06-01T00:00:00.000Z",
          valid_until: "2099-01-01T00:00:00.000Z",
          policy_blob: compiledPolicyBlob("agent-A", v),
        },
        payload_hash: "h",
        emitted_at: new Date().toISOString(),
        monotonic_seq: 0,

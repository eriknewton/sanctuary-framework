/**
 * Castle Wall Observe / Learn Allow-List v1 -- non-relaxable Tier-1 drift
 * guard. `castle_wall_observe_promote` must classify Tier 1 at BOTH policy
 * load (loader.ts) and runtime classification (gate.ts), no matter what a
 * hand-authored or maximally-permissive policy says, because both
 * force-lists spread the SAME `NON_RELAXABLE_CASTLE_WALL_OBSERVE_TIER1_OPERATIONS`
 * single source of truth.
 *
 * Modeled verbatim in shape on test/file-grant/non-relaxable-tier1.test.ts.
 */
...
describe("castle-wall-observe non-relaxable Tier 1, policy load", () => {
  for (const op of NON_RELAXABLE_CASTLE_WALL_OBSERVE_TIER1_OPERATIONS) {
...
describe("castle-wall-observe non-relaxable Tier 1, runtime classification", () => {
  for (const op of NON_RELAXABLE_CASTLE_WALL_OBSERVE_TIER1_OPERATIONS) {
...
describe("castle-wall-observe non-relaxable Tier 1, drift guard", () => {
  ...
  it("the shared set names exactly the castle-wall-observe promote operation", () => {
    expect([...NON_RELAXABLE_CASTLE_WALL_OBSERVE_TIER1_OPERATIONS]).toEqual(["castle_wall_observe_promote"]);
  });
});

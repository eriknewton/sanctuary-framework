  ...
  it("keeps the identity Ed25519 verification funnel rationale at the verifier", () => {
    ...
    expectNear(
      ...
      [
        ...
        "not a cross-language acceptance claim",
      ]
    );
    ...
    expectNear(source, "!isStrictEd25519PointEncoding(publicKey) ||", [
      ...
      "always the",
      "COFACTORED one",
      "for this TypeScript funnel",
    ]);

    // The exported helper's own doc comment must say who it is actually
    // shared with (this funnel only) rather than overclaiming every Ed25519
    // verifier in the server, since most callers of `ed25519.verify` in this
    // codebase bypass this funnel entirely.
    expectNear(
      source,

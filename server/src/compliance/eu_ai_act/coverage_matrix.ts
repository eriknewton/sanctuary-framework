...
const ROWS: CoverageRow[] = [
...

  {
    ...
    evidence_emitted:
      ...
      "Entries are persisted as AES-256-GCM authenticated ciphertext " +
      "under an HKDF-derived audit-log key and are linked into a " +
      "tamper-evident hash chain: each persisted entry carries a " +
      "sequence number and a prev_hash, and an entry_hash computed over " +
      "its sequence, prev_hash, timestamp, and ciphertext. Every " +
      "critical outcome is written durably (fsync plus read-after-write " +
      "byte verification) before the gate returns, so a completed gate " +
      "decision is on disk, not held only in memory. Entries are " +
      "queryable via monitor_audit_log or exportable in CEF/OCSF via " +
      "audit_export_siem. No tool call bypasses the audit path.",
    manual_carveout: null,
    last_reviewed_date: REVIEW_DATE,
    last_reviewed_by: REVIEWER,
    review_notes:
      "Re-verified against the current tree (server package v1.6.1) on " +
      "2026-07-05. router.ts routes every tool call through " +
      "gate.evaluate(); principal-policy/gate.ts appends to the audit " +
      "log on every outcome. UPDATED for the audit-chain hardening " +
      "(PR #274, merged 2026-05-16, plus follow-ups #290, #320, #396, " +
      "#461, #501): entries are now linked by a per-entry prev_hash " +
      "chain (PersistedAuditEnvelopeV2 in operational/audit-log.ts, " +
      "schema version 2) over AES-256-GCM ciphertext, and periodic " +
      "checkpoint records carry a SHA-256 root over the covered entry " +
      "hashes, Ed25519-signed when a signing identity is available " +
      "(unsigned and marked as such otherwise). The signature binds the " +
      "checkpoint root, not each individual entry; per-entry integrity " +
      "is the AES-256-GCM authentication tag plus the prev_hash link. " +
      "Durability is now enforced: critical gate outcomes use " +
      "appendCritical, which awaits a durable write (file plus " +
      "directory fsync) and a read-after-write round-trip verification " +
      "and throws AuditPersistenceError on any storage, disk-full, " +
      "permission, or torn-write failure. The earlier fire-and-forget " +
      "caveat no longer applies to critical entries; only best-effort " +
      "telemetry appends (for example injection_detected) use the " +
...

  {
    ...
    evidence_emitter: ["monitor_audit_log", "audit_export_siem"],
    evidence_emitted:
      "Auto-filled: Sanctuary's audit log persists entries by default " +
      "until configured size or entry caps trigger authenticated " +
      "rotation (defaults: 100,000 entries / 100 MB), at which point " +
      "the oldest entries are pruned behind a master-MAC'd rotation " +
      "anchor so the cut is tamper-evident. Enterprises retaining " +
      "beyond those caps must archive via audit_export_siem, which is " +
      "exportable at any time for archival to enterprise storage.",
    manual_carveout:
    ...
    review_notes:
      ...
      "exists in Sanctuary; the on-disk log grows until the configured " +
      "size/entry caps trigger authenticated rotation (defaults " +
      "100,000 entries / 100 MB), so a high-volume deployment must " +
      "archive via audit_export_siem to satisfy a retention window " +
      "beyond those caps. The declared policy is an enterprise artefact.",
  },
...

  {
    ...
    evidence_emitted:
      "Auto-filled: signed SHR + tamper-evident, checkpoint-signed " +
      "audit chain + " +
...

  {
    ...
    evidence_emitted:
      ...
      "periodic checkpoint records carrying a SHA-256 root over the " +
      "covered entry hashes, Ed25519-signed when a signing identity is " +
      "available; a rotation anchor authenticated by a master-derived " +
    ...
    review_notes:
      "Re-verified against the current tree (server package v1.6.1) on " +
      "2026-07-05. UPDATED for the audit-chain hardening wave (PR #274 " +
      "merged 2026-05-16, plus #290, #320, #396, #461, #501). The three " +
      "items earlier drafts had corrected AWAY are now present and must " +
      "be stated: (1) the audit log now carries a SHA-256 root over " +
      "each checkpoint's entry-hash set (computeAuditRoot in " +
      "audit/chain.ts), so integrity coverage is no longer " +
      "state-store-only; (2) checkpoint records are Ed25519-signed over " +
      "domain-separated canonical JSON when a signing identity is " +
      "available (records are marked unsigned with a reason otherwise), " +
      "so there is non-repudiation at checkpoint granularity, though " +
      "NOT a distinct signature on each individual entry; (3) entries " +
      "are chained by prev_hash (PersistedAuditEnvelopeV2, schema " +
      "version 2), so a tamper-evident transcript does now exist. The " +
      "precise bound: per-entry tamper-evidence is the AES-256-GCM " +
      "authentication tag plus the prev_hash link (keyed to the " +
      "master); Ed25519 non-repudiation is at checkpoint granularity " +
      "over the Merkle-style root, and is conditional on a signing " +
      "identity being present at checkpoint time. Non-repudiation " +
      "against a compromised-master-key insider who forges the " +
      "checkpoint signer is still out of scope; the row survives as " +
      "full because Art. 15(5) targets unauthorised third parties, and " +
      "the chain plus signed checkpoint plus master-MAC'd rotation " +
      "anchor are resilient against that threat model.",
  },
...

  {
    ...
    evidence_emitted:
      "Auto-filled: Sanctuary's audit log persists entries by default " +
      "until configured size or entry caps trigger authenticated " +
      "rotation (defaults: 100,000 entries / 100 MB); it is exportable " +
      "for archival at any time via audit_export_siem in CEF/OCSF " +
      "formats suitable for long-term SIEM retention. A deployment " +
      "whose six-month window exceeds those caps must archive via that " +
      "export.",
    manual_carveout:
...
export const COVERAGE_MATRIX_V1: CoverageMatrix = {
  ...
  notes: [
    ...
    "Every 'full' row was individually verified against source. The " +
      "audit-log rows (Article 12(1), Article 15(5) first subparagraph) " +
      "were re-verified against the current tree (server package " +
      "v1.6.1) on 2026-07-05 to reflect the tamper-evident audit-chain " +
      "hardening (PR #274 and follow-ups #290, #320, #396, #461, #501). " +
      "The remaining rows carry their original 2026-04-10 verification " +
      "against v0.7.0 source. See per-row review_notes for verification " +

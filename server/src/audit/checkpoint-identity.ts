 ...
 *
 * Failure semantics (second-gate HIGH fix): proven identity ABSENCE and
 * identity-material FAILURE are different states and must never collapse.
 * A store that provably holds no identity yields `null`/`undefined` (the
 * audit log serializes that as an honest `unsigned` checkpoint, and reports
 * an unresolved signer as an explicit integrity finding). A store whose
 * identity material exists but cannot be read, parsed, or decrypted, or
 * whose signing operation fails, throws `CheckpointSignerUnavailableError`:
 * treating corruption as "no identity" was empirically a silent fail-open
 * (a corrupted identity record demoted checkpoints to unsigned with zero
 * findings). The audit log converts the throw into a loud, distinct state,
 * never into the honest-absence path.
 */
 ...
 * error and must reserve the honest `unsigned` path for proven absence.
 */
 ...
 * close an import cycle through the audit log.
 */
const STRUCTURAL_NAMESPACE_REFUSAL_CODE = "SANCTUARY_AUDIT_NAMESPACE_UNSUPPORTED";

function isStructuralNamespaceRefusal(err: unknown): boolean {
  return (
    typeof err === "object" &&
    err !== null &&
 ...
 * or no record exists under it; "unreadable" means a record EXISTS but does
 * not parse, decrypt under this fortress's identity-encryption key, or claim
 * the id it is stored under. An unreadable record proves nothing about the
...
async function resolveSigningIdentity(
  ...
  if (pointerRaw !== null) {
    ...
    if (primary.status === "unreadable") {
      // The second-gate probe: a valid pointer to a corrupted record. Falling
      // through to the scan (or to null) here is exactly the silent
      // fail-open; the record exists, so absence is disproven.
      throw new CheckpointSignerUnavailableError(primary.detail);
    }
  ...
  for (const entry of entries) {
    ...
    if (read.status === "unreadable") {
      throw new CheckpointSignerUnavailableError(read.detail);
    }
 ...
 * honest-unsigned and the loud signer-error paths respectively.
 *

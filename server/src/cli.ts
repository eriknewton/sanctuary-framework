...
async function main(): Promise<void> {
  ...
  if (args[0] === "memory_provenance_mark_bad_signer") {
    const { runMemoryProvenanceMarkBadSignerCommand } = await import("./cli/memory-archive.js");
    return drainAndExit(await runMemoryProvenanceMarkBadSignerCommand({ argv: args.slice(1) }));
  }

  if (args[0] === "memory_provenance_clear_bad_signer") {
    const { runMemoryProvenanceClearBadSignerCommand } = await import("./cli/memory-archive.js");

...
| evidence-pack | Compliance (law-firm evidence pack) | canonical (slice 1: walking skeleton) | The quarterly law-firm Evidence Pack generator: a NEW calendar-quarter aggregation over audit activity (`aggregate.ts` buckets `AuditEntry[]` into a `YYYY-Qn` window, inclusive-start/exclusive-end, and counts by decision category derived from the shipped `gate_*` operation prefixes + the cross-harness approval resolution op) plus covered-window shortfall detection (`detectShortfall` flags + discloses when size-based FIFO retention may not cover the full quarter, distinguishing at-cap pruning from genuine inactivity), the CNA-five-question section renderers with honest coverage-basis + scope-and-limits language and clearly-labeled placeholders for later slices (`sections.ts`), a signed manifest reusing the EU AI Act bundle's Ed25519 discipline (`signer.ts`), and the PDF render via the reused zero-dep writer (`renderMarkdownDocumentsToPdf` in `compliance/eu_ai_act/pdf.ts`). CLI `sanctuary evidence-pack generate --quarter 2026-Q3 --firm "Name"` (`cli.ts`). Pure core (`generate.ts` `buildEvidencePack`) is I/O-free + hermetically tested | compliance/eu_ai_act (the regulatory bundle whose PDF writer + signing pattern this reuses); audit (posture score, not a period report). The pure aggregation NEVER reads a real fortress | Yes | working product name `Sanctuary Evidence Pack` (cover copy); output filenames `00_pack_manifest.json`/`01_evidence_pack.md`/`evidence-pack.pdf`; CLI verb `evidence-pack generate`; the PDF is deliberately UNSIGNED (integrity lives in the manifest); no per-rule per-flow claim (thesis-gate honesty bound) |

...
| entitlement | Cross-cutting (paid-fleet tier gating + license issuance) | canonical (verify/resolve core + PR-1 issuance) | Offline Ed25519 entitlement-token verify + tier resolution for the paid fleet control plane, PLUS the PR-1 issuance side: a v2 signed-license schema (`EntitlementClaimsV2`  -  carries `entitledCount`/`featureFlags`/`pricingUnit`/`period`/`graceUntil`/`licenseId`/`issuer`, all signed), the tamper-evident append-only issuance ledger (`ledger.ts` hash-chain + per-row revocation signature + signed VERSIONED head anchor carrying a monotonic `generation`; `ledger-io.ts` load/save + an advisory `withLedgerMutationLock`; `ledger-antirollback.ts` the EXTERNAL master-MAC'd generation anchor in `_meta` that catches an old/empty-snapshot rollback the within-file check cannot), and the `sanctuary license issue/list/revoke` CLI (`cli/license.ts`). FAIL-CLOSED: absent/invalid/expired/tampered/malformed all resolve to the COMMUNITY tier and never grant a paid tier; a tampered OR rolled-back ledger reports `tampered` and is never trusted. `list` pins the issuer key from the fortress (NOT the ledger file) in verified mode, and labels an unverified (no-passphrase) read `UNVERIFIED` rather than claiming valid. NOT built yet: enforcement wiring (PR-2), the distributed revocation-push rail (PR-3), a ledger re-baseline path (restore-attest analogue), billing/self-serve | NOT an enforcement gate (a caller consumes the resolved tier / signed fields); NOT identity/attestation (verifies an issuer-signed grant, not an agent identity); ledger `revoke` is LOCAL bookkeeping, NOT the pushed revocation list | Yes | domain labels `sanctuary.fleet.entitlement.v1` (FROZEN) + `sanctuary.fleet.entitlement.v2` (FROZEN); ledger revocation domain `sanctuary.fleet.ledger.revocation.v1`; ledger head domain `sanctuary.fleet.ledger.head.v1` (v1/v2 versioned by field-set, label frozen); external generation-anchor MAC domain + `_meta` key `fleet-license-ledger-generation-anchor-v1` (additive: absent reads as gen 0; monotonic write-guard; wrong-key/tampered fails toward tampered, never bricks); closed tier set `ENTITLEMENT_TIERS` (community/team/fleet/enterprise) with `COMMUNITY_TIER` as the floor; `resolveEntitlement` never throws + never returns key material; issuer secret decrypted only transiently in `sign()` (never printed/logged); Ed25519-over-canonical-JSON, offline, no network |

...
| entitlement | Cross-cutting (paid-fleet tier gating + license issuance + PR-2 activation/enforcement) | canonical (verify/resolve core + PR-1 issuance + PR-2 activate/enforce) | Offline Ed25519 entitlement-token verify + tier resolution for the paid fleet control plane, PLUS the PR-1 issuance side: a v2 signed-license schema (`EntitlementClaimsV2`  -  carries `entitledCount`/`featureFlags`/`pricingUnit`/`period`/`graceUntil`/`licenseId`/`issuer`, all signed), the tamper-evident append-only issuance ledger (`ledger.ts` hash-chain + per-row revocation signature + signed VERSIONED head anchor carrying a monotonic `generation`; `ledger-io.ts` load/save + an advisory `withLedgerMutationLock`; `ledger-antirollback.ts` the EXTERNAL master-MAC'd generation anchor in `_meta` that catches an old/empty-snapshot rollback the within-file check cannot), and the `sanctuary license issue/list/revoke` CLI (`cli/license.ts`). PLUS the PR-2 activation + node-count ENFORCEMENT: `activation.ts` (verify a pasted license through the shipped core, persist it in a master-MAC'd `_meta` record, and RE-RESOLVE it against the current clock on every read  -  fail-closed to community on absent/tampered/wrong-key/expired) and `fleet-cap.ts` (the pure NODE-COUNT gate: `resolveFleetCap` maps a resolution + grandfather baseline to a cap, `applyFleetCap` drops nodes beyond the cap from the CENTRAL roster). The gate is NODE COUNT on the central management surface ONLY; it NEVER gates a node's Castle Wall, its local dashboard, or free policy-push, and preserves the roster's `policy_distribution` rail verbatim. Grandfathering: an existing >5-node fleet keeps its count free indefinitely via a per-fleet baseline. FAIL-CLOSED: absent/invalid/expired/tampered/malformed all resolve to the COMMUNITY tier/floor and never grant a paid tier; a tampered OR rolled-back ledger reports `tampered` and is never trusted. `list` pins the issuer key from the fortress (NOT the ledger file) in verified mode, and labels an unverified (no-passphrase) read `UNVERIFIED` rather than claiming valid. NOT built yet: the distributed revocation-push rail + operator-visible downgrade log/banner (PR-3), a ledger re-baseline path (restore-attest analogue), billing/self-serve | NOT an enforcement gate for SECURITY (only central-management node count; never the wall/local-dashboard/policy-push); NOT identity/attestation (verifies an issuer-signed grant, not an agent identity); ledger `revoke` is LOCAL bookkeeping, NOT the pushed revocation list; the wrap-dashboard wiring lives in `wrap/fleet-roster-provider.ts`, not here | Yes | domain labels `sanctuary.fleet.entitlement.v1` (FROZEN) + `sanctuary.fleet.entitlement.v2` (FROZEN); ledger revocation domain `sanctuary.fleet.ledger.revocation.v1`; ledger head domain `sanctuary.fleet.ledger.head.v1` (v1/v2 versioned by field-set, label frozen); external generation-anchor MAC domain + `_meta` key `fleet-license-ledger-generation-anchor-v1` (additive: absent reads as gen 0; monotonic write-guard; wrong-key/tampered fails toward tampered, never bricks); activation MAC domain `sanctuary.fleet.activation.v1` + `_meta` key `fleet-activation-v1` (additive: absent reads as no-license; wrong-key/tampered fails toward community, never grants); `COMMUNITY_FREE_NODE_CAP=5`; closed tier set `ENTITLEMENT_TIERS` (community/team/fleet/enterprise) with `COMMUNITY_TIER` as the floor; `resolveEntitlement`/`resolveActivation`/`activateFleet` never throw + never return key material; issuer secret decrypted only transiently in `sign()` (never printed/logged); Ed25519-over-canonical-JSON, offline, no network |
...
| wrap | CLI (install-time onboarding) | canonical | The `sanctuary wrap`/`init` onboarding CLI: detect + back up + rewrite a harness's MCP config so Sanctuary becomes the upstream gateway; passphrase/recovery-key/keychain custody; Hermes YAML emitter (`hermes-yaml.ts`) guarded by a PyYAML parse-parity check (`hermes-yaml-parse-parity.ts`) that refuses to edit config.yaml when the line-scanner's view disagrees with a real PyYAML parse or when that parser cannot run (fail-closed); the disk-backed fleet-roster provider (`fleet-roster-provider.ts`) the wrap dashboard passes to its fleet panel  -  now the PR-2 ENFORCED provider (`buildEnforcedWrapFleetRosterProvider`, read-only roster + node-count cap from the persisted activation, fail-closed to community) plus the activation handler (`buildWrapFleetActivationHandler`) feeding `POST /api/fleet/activate`; both resolve the pinned issuer key from the default operator identity and NEVER gate the wall/local-dashboard/policy-push | supervisor (runtime process); the Layer-2 managed-child path (`wrap --tier-b`); castle-wall; `principal-policy/fleet-roster.ts` (the presenter `fleet-roster-provider.ts` feeds); `entitlement/` (`applyFleetCap`/`resolveActivation`/`activateFleet` for PR-2 enforcement) | Yes | `WRAP_META_FILENAME="wrap-meta.json"` + the legacy read-only `LEGACY_WRAP_META_FILENAME` constant (its value is a retired-vocabulary filename kept read-only for back-compat; the ONLY permitted carrier of the retired term; removing it breaks unwrap of old releases) + surface-scoped `wrap-meta-<12-hex-surface-tag>.json` slots (per-surface unwrap pointers; written when a wrap would clobber another surface's canonical meta, scanned by every reader after the canonical + legacy filenames); env `SANCTUARY_FORTRESS_PATH`/`SANCTUARY_STORAGE_PATH`; verbs `wrap`/`init`/`--unwrap` |

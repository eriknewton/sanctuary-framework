...
TypeScript MCP server: 59 module directories plus a small set of root files. It is the in-process
...
## MODULE INDEX TABLE (59 modules)

All 59 module directories under `server/src` are listed. Status legend: **canonical** = a real,
...
| wrap | CLI (install-time onboarding) | canonical | The `sanctuary wrap`/`init` onboarding CLI: detect + back up + rewrite a harness's MCP config so Sanctuary becomes the upstream gateway; passphrase/recovery-key/keychain custody; Hermes YAML emitter (`hermes-yaml.ts`) guarded by a PyYAML parse-parity check (`hermes-yaml-parse-parity.ts`) that refuses to edit config.yaml when the line-scanner's view disagrees with a real PyYAML parse or when that parser cannot run (fail-closed); the read-only disk-backed fleet-roster provider (`fleet-roster-provider.ts`) the wrap dashboard passes to its fleet panel | supervisor (runtime process); the Layer-2 managed-child path (`wrap --tier-b`); castle-wall; `principal-policy/fleet-roster.ts` (the presenter `fleet-roster-provider.ts` feeds) | Yes | `WRAP_META_FILENAME="wrap-meta.json"` + the legacy read-only `LEGACY_WRAP_META_FILENAME` constant (its value is a retired-vocabulary filename kept read-only for back-compat; the ONLY permitted carrier of the retired term; removing it breaks unwrap of old releases) + surface-scoped `wrap-meta-<12-hex-surface-tag>.json` slots (per-surface unwrap pointers; written when a wrap would clobber another surface's canonical meta, scanned by every reader after the canonical + legacy filenames); env `SANCTUARY_FORTRESS_PATH`/`SANCTUARY_STORAGE_PATH`; verbs `wrap`/`init`/`--unwrap` |
...
| dashboard | Enforcement-surface | versioned-frozen (v1) | The Sovereignty Dashboard: a localhost "hero-shield" status UI (default port 3501) showing L1-L4 state, activity feed, pending approvals. Hosts `v1_1/` serving `/v1.1`, `/api/hub/*`, `/api/identities`. Read-only fleet-roster panel (SEE-not-manage: no admit/revoke/rotate seam) presents `principal-policy/fleet-roster.ts`: the wrap ("Protect") CLI wires a REAL disk-backed provider (`wrap/fleet-roster-provider.ts`, load-only over the at-rest federation trust root + durable revocation projection) into BOTH `GET /api/fleet/roster` AND the posture route `GET /api/posture/fleet` that the served `posture-home-html` fleet panel fetches; honest absent roster when no federation is provisioned | console (control surface); `principal-policy/dashboard.ts` (approval channel); `principal-policy/fleet-roster.ts` (the presenter the panel serves); `wrap/fleet-roster-provider.ts` (the read-only disk-backed provider the wrap CLI passes). NOTE: root `dashboard-standalone.ts` actually serves the principal-policy channel, NOT this dir | Yes | port 3501; routes `/v1.1`, `/api/hub/*`, `/api/identities`, read-only `/api/fleet/roster`; UI token `HERO_COPY="Your agent is protected."` + `id="hero-copy"`; the snake_case subdir `v1_1/` doubles as a frozen path token |
...
Gaps note: none. All 59 module directories under `server/src` (verified by `ls`) have a row above,
...
**Status today: 51 of 59 modules have a barrel** (backfilled 2026-06-14; file-grant added its own
2026-07-07; evidence-pack shipped with its own barrel 2026-07-10). This is additive (adding a

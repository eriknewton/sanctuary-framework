...
| entitlement | Cross-cutting (paid-fleet tier gating + license issuance) | canonical (verify/resolve core + PR-1 issuance) | Offline Ed25519 entitlement-token verify + tier resolution for the paid fleet control plane, PLUS the PR-1 issuance side: a v2 signed-license schema (`EntitlementClaimsV2`  -  carries `entitledCount`/`featureFlags`/`pricingUnit`/`period`/`graceUntil`/`licenseId`/`issuer`, all signed), the tamper-evident append-only issuance ledger (`ledger.ts` hash-chain + per-row revocation signature; `ledger-io.ts` load/save), and the `sanctuary license issue/list/revoke` CLI (`cli/license.ts`). FAIL-CLOSED: absent/invalid/expired/tampered/malformed all resolve to the COMMUNITY tier and never grant a paid tier; a tampered ledger reports `tampered` and is never trusted. NOT built yet: enforcement wiring (PR-2), the distributed revocation-push rail (PR-3), billing/self-serve | NOT an enforcement gate (a caller consumes the resolved tier / signed fields); NOT identity/attestation (verifies an issuer-signed grant, not an agent identity); ledger `revoke` is LOCAL bookkeeping, NOT the pushed revocation list | Yes | domain labels `sanctuary.fleet.entitlement.v1` (FROZEN) + `sanctuary.fleet.entitlement.v2` (FROZEN); ledger revocation domain `sanctuary.fleet.ledger.revocation.v1`; closed tier set `ENTITLEMENT_TIERS` (community/team/fleet/enterprise) with `COMMUNITY_TIER` as the floor; `resolveEntitlement` never throws + never returns key material; issuer secret decrypted only transiently in `sign()` (never printed/logged); Ed25519-over-canonical-JSON, offline, no network |

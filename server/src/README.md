...
**Status today: 28 of 54 modules have a barrel** (`principal-policy`, `audit`, `transparency`, and
`federation` were added 2026-06-14). This is being backfilled additively (adding a barrel never
changes existing deep-import call sites). The highest-leverage remaining gaps are `l2-operational`
(118 importers reach `audit-log.js` directly - the biggest rename blast radius) and `shr`. Several
modules are barrel-less for honest reasons recorded in their rows; do not add a top-level
`contracts/index.ts` (it would falsely merge two frozen wire versions).

 ...
 * persisted poll cursor, and emits the result on stdout. Operator-
 * debuggable from the shell; also useful for any harness-level glue
 * code that wants to stream pending operator messages without spawning
 * a chat-server subprocess.
 *
 ...
 * Argon2id cost once per invocation; per-call latency is ~500-1500ms
 * on a modern Mac. Acceptable for v1.2.0; v1.3 may introduce a shared
 * cocoon daemon to amortize key derivation across calls.
 */

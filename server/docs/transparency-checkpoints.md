...
Exit codes: 0 PASS, 1 FAIL, 2 usage error. The report never says "best effort passed": every failed check is a specific finding, and everything the run could not check is listed under `not_checked`.
...
3. **Audit-log commitment.** Each checkpoint commits to the entire surviving audit hash chain at its emission moment. Anyone who later runs `--against-log` on the host can detect tail truncation (live head behind a signed head), in-window deletion, and content alteration relative to that commitment.
4. **Policy stability or change.** Equal `policy.rules_sha256` values across checkpoints prove the policy input files did not change between them; a changed hash proves the policy changed. (What the policy says is deliberately not disclosed.)
5. **Enforcement activity counts.** The signed allowed/blocked totals and opaque per-rule counts for the covered window. On the host, with the fortress passphrase, these counts are recomputable from the decrypted log and the verifier compares them.
6. **Fork detection across observers.** Two checkpoints with the same counter but different contents, obtained by different observers, prove the operator presented divergent histories.
...
3. **Completeness of history.** An operator who stops publishing produces a shorter, still-valid chain. Freshness and completeness come from publication cadence and from cross-checking counters across independently obtained checkpoints, not from any single bundle.

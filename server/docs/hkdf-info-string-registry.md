...
| `audit-signing-latch` | `server/src/operational/audit-log.ts` | (literal) | MAC key for the checkpoint-signing downgrade latch (one-way memory that the fortress signs its checkpoints). **(MAC/crypto-domain label, not a decryptable store.)** |

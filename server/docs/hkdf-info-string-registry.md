...
| `federation-sync-state` | `server/src/v1/federation-sync-state-store.ts` (`FEDERATION_SYNC_STATE_STORE_HKDF_INFO`) | `FEDERATION_SYNC_STATE_STORE_HKDF_INFO` | Encrypts the `_federation/sync-state-v1` durable peer-sync security state (Federation 3/3b P0): per-sender accepted high-water + outbound high-water + folded node-revocation projection + (PR-A durable fleet membership) the grow-only **node roster** (the authoritative source of the paid node-count; minimal per-node reconstruction fields only, NO secrets/private keys). Makes anti-replay AND revocation AND fleet membership survive a daemon restart (captured envelope rejected post-restart; revoked node stays revoked across reboot; the node-count no longer resets to zero on reboot); fail-closed on a corrupt record (a disk-forged extra node fails the AEAD tag). **ALSO** the MAC key for two `_meta` markers (F1 re-gate, §7 reuse: NO new HKDF label): the guardian **established sentinel** (`_meta/federation-guardian-requirement-established-v1`, domain `sanctuary.federation.guardian-requirement.established.v1`, a dataless MAC) and the guardian **anti-rollback anchor** (`_meta/federation-guardian-antirollback-anchor-v1`, domain `sanctuary.federation.guardian-antirollback-anchor.v1\n`, a `{marker,data,mac}` record MAC'd over `canonicalJson(data)`). Domain separation comes from the two distinct MAC-domain strings (see section E), exactly as `custody-rollback-freeze` vs `custody-epoch-witness` are separated. Both are handled under master rotation by the `master-rotation.ts` classes `federation-guardian-established` (inline re-derive of the dataless MAC) and `federation-guardian-antirollback` (`restampMacRecord`). **(scan-reconciled 2026-06-24; amended 2026-07-05 for the F1 re-gate MAC-marker reuse; decryptable store; frozen at-rest contract.)** |
| `v1-operator-authorization-spent-set` | `server/src/v1/operator-authorization-spent-store.ts` (`OPERATOR_AUTHORIZATION_SPENT_STORE_HKDF_INFO`, via `durable-spent-set-store.ts`) | `OPERATOR_AUTHORIZATION_SPENT_STORE_HKDF_INFO` | Encrypts the `_operator_authorizations/spent-v1` durable single-use OPERATOR_SIGNED authorization spent-set. The v1 agents write routes use this store on single-node and federated dashboards, so authorization replay denial does not depend on federation provisioning; failed or corrupt persistence fails closed. **(scan-reconciled 2026-08-09; decryptable store; frozen at-rest contract.)** |
...
| `l4-known-signers` | `server/src/reputation/known-signers-store.ts:79` | (literal) | Encrypts persisted per-DID verified signer keys (Exit V2 drill F2, 2026-08-22/23), so a later export can rebuild a signed `known_signers` table. |
...
| `memory-checkpoint-v1` | `server/src/memory-checkpoint/store.ts:43` | `MEMORY_CHECKPOINT_PURPOSE` | Encrypts local StateStore export-bundle checkpoint snapshots under `<fortress>/checkpoints/<id>/bundle.enc`; `record.json` is non-secret metadata only. **(scan-reconciled 2026-08-03; decryptable store; no network/fleet-sync path.)** |
...
| `audit-signing-latch-v2` | `server/src/operational/audit-log.ts` (`AUDIT_SIGNING_LATCH_V2_MAC_PURPOSE`) | (literal) | MAC key for the v2 checkpoint-signing latch (IC-05-DG): one-way forward commitment that this fortress signs its audit checkpoints from a given sequence. Fresh v2 string; the unmerged #1243 branch's `audit-signing-latch` label carried different (historical) semantics and is deliberately NOT reused. Restamped across master rotation by `convertAuditAnchors`. **(MAC/crypto-domain label, not a decryptable store.)** |
| `audit-signing-head` | `server/src/operational/audit-log.ts` (`AUDIT_SIGNING_HEAD_MAC_PURPOSE`) | (literal) | MAC key for the checkpoint-signing head (IC-05-DG): fixed-size authenticated record carrying the signer-failure incident ring, the monotone committed tip and floor witnesses, and the latched recovery/downgrade markers. Restamped across master rotation by `convertAuditAnchors`. **(MAC/crypto-domain label, not a decryptable store.)** |
...
| `audit-store-split-boundary` | `server/src/operational/audit-log.ts` (`deriveAuditStoreSplitBoundaryMacKey`) | (literal) | MAC key for the F2 Option A (2026-07-14) fortress audit store split-boundary record, the marker that seals a legacy (pre-writer-split) `_audit` chain and hands the root daemon its own `_audit-daemon` chain. New label, additive; does not touch any existing store. |
| `enforcement-export-cursor` | `server/src/castle-wall/export/cursor.ts` (`FileExportCursorStore`) | (literal) | MAC key authenticating the enforcement-export durable cursor (`<fortress>/state/cortex-export-cursor.json`, schema `sanctuary.enforcement-export-cursor.v2`); the record is MAC'd + chain-identity bound so a hand-written/tampered/stale-high cursor is discarded fail-LOUD rather than silently zero-forwarding and blinding the off-box console. New label, additive (2026-07-16 sweep fix 1); does not touch any existing store. **(MAC/crypto-domain label.)** |
...
| `fleet-ledger-generation-mac` | `server/src/entitlement/ledger-antirollback.ts` | `LEDGER_GENERATION_ANCHOR_MAC_PURPOSE` | MAC key for the external monotonic fleet-license-ledger generation anchor (`_meta` key `fleet-license-ledger-generation-anchor-v1`). Mirrors the custody epoch-witness anti-rollback pattern; an absent anchor reads as generation 0 (additive). **(scan-reconciled 2026-07-05; MAC/crypto-domain label.)** |
| `fleet-activation-mac` | `server/src/entitlement/activation.ts` | `FLEET_ACTIVATION_MAC_PURPOSE` | MAC key for the fleet ACTIVATION record (PR-B; `_meta` key `fleet-activation-v1`) authenticating the operator's persisted pasted license + grandfather baseline. Mirrors the custody epoch-witness anti-rollback pattern; absent reads as "no license" and a tampered/wrong-key record fails closed to community (never grants). **(scan-reconciled 2026-07-06; MAC/crypto-domain label.)** |
| `fleet-revocation-list-mac` | `server/src/entitlement/revocation-list.ts` | `REVOCATION_LIST_MAC_PURPOSE` | MAC key for the fleet REVOCATION-LIST record (PR-3; `_meta` key `fleet-revocation-list-v1`) authenticating the operator-pushed, issuer-signed revoked-license-id set + monotonic version. Distinct from the revocation-list SIGN action `sanctuary.fleet.revocation-list.v1`; the corrupt-list ENFORCEMENT posture is fail-closed (a present-but-corrupt list drops a paid grant to Community; an absent list keeps it). **(scan-reconciled 2026-07-06; MAC/crypto-domain label.)** |
| `fleet-revocation-version-mac` | `server/src/entitlement/revocation-antirollback.ts` | `REVOCATION_VERSION_ANCHOR_MAC_PURPOSE` | MAC key for the EXTERNAL monotonic fleet-revocation-list version anchor (PR-3 fix; `_meta` key `fleet-revocation-list-version-anchor-v1`). Mirrors the fleet-license-ledger generation anchor so a corrupt stored revocation list cannot roll the monotonic version floor back to 0 and re-apply an old signed list to un-revoke licenses; effective floor is `max(storedListVersion, anchorVersion)`. **(scan-reconciled 2026-07-06; MAC/crypto-domain label.)** |
| `fleet-downgrade-log-mac` | `server/src/entitlement/downgrade-log.ts` | `DOWNGRADE_LOG_MAC_PURPOSE` | MAC key for the operator-visible fleet DOWNGRADE LOG (PR-3; `_meta` key `fleet-downgrade-log-v1`), an append-only bounded record of tier/cap transitions with reason + affected node ids (no secrets); tamper-evident under the master. **(scan-reconciled 2026-07-06; MAC/crypto-domain label.)** |
...
| `sdw-owner-pin-mac` | `server/src/sdw/write-gate.ts` | `SDW_OWNER_PIN_MAC_INFO` | MAC key authenticating the durable one-owner-per-fortress SDW binding. **(IC-16, 2026-09-01; MAC/crypto-domain label.)** |
| `sdw-memory-passage-id-v1` | `server/src/sdw/adapters/sdw-memory-backend.ts` | `MEMORY_PASSAGE_ID_HKDF_INFO` | MAC key deriving opaque SDW memory passage ids from private caller labels, so a storage key (and therefore an on-disk directory entry) never carries the memory topic name in cleartext. Encrypts nothing; changing it re-derives every id and orphans previously mirrored passages. **(scan-reconciled 2026-08-07; MAC/crypto-domain label.)** |
| `sdw-memory-bad-signers-v1` | `server/src/sdw/memory-provenance-bad-signers.ts` | `KEY_PURPOSE` | Encrypts authenticated, bounded foreign bad-signer marks keyed by exact DID plus raw-key SHA-256 fingerprint. **(scan-reconciled 2026-08-24; decryptable store.)** |

...
| `fleet-revocation-list-mac` | `server/src/entitlement/revocation-list.ts` | `REVOCATION_LIST_MAC_PURPOSE` | MAC key for the fleet REVOCATION-LIST record (PR-3; `_meta` key `fleet-revocation-list-v1`) authenticating the operator-pushed, issuer-signed revoked-license-id set + monotonic version. Distinct from the revocation-list SIGN action `sanctuary.fleet.revocation-list.v1`; a corrupt local list reads not-revoked (never a DoS strip). **(scan-reconciled 2026-07-06; MAC/crypto-domain label.)** |

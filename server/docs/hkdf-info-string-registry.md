...
| `fleet-activation-mac` | `server/src/entitlement/activation.ts` | `FLEET_ACTIVATION_MAC_PURPOSE` | MAC key for the fleet ACTIVATION record (PR-2; `_meta` key `fleet-activation-v1`) authenticating the operator's persisted pasted license + grandfather baseline. Mirrors the custody epoch-witness anti-rollback pattern; absent reads as "no license" and a tampered/wrong-key record fails closed to community (never grants). **(scan-reconciled 2026-07-05; MAC/crypto-domain label.)** |

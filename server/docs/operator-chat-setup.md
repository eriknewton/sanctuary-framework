...
The remaining piece is the wake mechanism: when does the agent's runtime decide to poll? That is harness-level behaviour, not MCP-server behaviour. Sanctuary handles Claude Code automatically through a CLAUDE.md augmentation. Other harnesses use the same augmentation pattern, pinned manually into the harness's instruction surface. This document covers both.

## Quick reference

| Harness | Wake mechanism | Setup step |
|---|---|---|
| Claude Code | CLAUDE.md augmentation (autonomous voluntary poll on every turn) | `sanctuary wrap --claude-code --install-hooks` (default-on; opt-out with `--no-install-hooks`) |
...
- For Claude Code with the augmentation installed: any time Claude finishes a turn (after operator typed in Claude Code, or after a tool call cycle ends), Claude voluntarily calls `chat/poll_inbox` and replies via `chat/send_reply` if pending messages are present. End-to-end latency: ~3-5 seconds when Claude is in or just-finished a turn.
...
## Claude Code (autonomous via CLAUDE.md augmentation)

`sanctuary wrap --claude-code --install-hooks` appends a Sanctuary-tagged section to `~/.claude/CLAUDE.md`. Claude Code reads that file on session start as user-global instruction context. The augmentation describes the `sanctuary-chat` MCP tools and tells Claude to call them voluntarily after every turn. Claude treats the instruction as your own context (the trusted shape), not as an external mid-stream injection.

The section is delimited by exact fenced HTML comment markers (`<!-- SANCTUARY:CHAT-AUGMENTATION-V1 -->` and the matching close). Re-running `sanctuary wrap --install-hooks` is a true no-op when the V1 marker is already present. Operator content above and below the section is preserved byte-for-byte. A pre-write backup at `~/.claude/CLAUDE.md.sanctuary-backup` snapshots the prior file on every modify-existing install.

Latency: ~3-5 seconds after Claude turn-end. If Claude is sitting idle at the prompt, type any character (e.g. `/help`) to wake the session into a turn.

Opt-out: `sanctuary wrap --claude-code --no-install-hooks`. Use the Claude Code system-prompt augmentation below as the manual fallback (paste the snippet into `~/.claude/CLAUDE.md` between operator-controlled markers, or anywhere in the file).

Multi-fortress: the augmentation body is invariant across wraps. Per-fortress binding lives in the `sanctuary-chat` MCP-config env block, not in CLAUDE.md. The most-recent wrap binds `sanctuary-chat` to the most-recent fortress; CLAUDE.md remains stable.
...
After pasting the augmentation (or installing it via wrap for Claude Code):

1. Open the Sanctuary dashboard, click the wrapped agent in the agent list. The chat surface opens directly (PR #98 click-to-chat removed the redundant Tier 1 modal).
2. Type "what tools are available to you" and submit.
3. Switch to your harness. Type any prompt to bring the harness into a turn (Claude Code with the augmentation: any prompt; other harnesses: any normal prompt).
4. Watch the Sanctuary dashboard chat. The agent's reply should render within a few seconds of its turn ending.
...
- For Claude Code with the augmentation: re-run `sanctuary wrap --claude-code --install-hooks` and check the console line. The output should say `Sanctuary chat augmentation installed` or `already present`. If it says `chat augmentation install failed`, the error message contains the cause. Also verify `cat ~/.claude/CLAUDE.md | grep SANCTUARY:CHAT-AUGMENTATION-V1` returns the open and close marker lines.
...
- The augmentation snippet you pasted (or the `sanctuary wrap` console line for Claude Code).
